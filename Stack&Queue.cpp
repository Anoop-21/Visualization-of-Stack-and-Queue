...
void frntpge() {
	mouseval = -30;
	char by[] = "Under the guidance of Punit RP", title[]="      Stack And Queue",ta[]="Anoop Choodamani         4nm19cs022";
	char sabu[] = "Chinmay Sandur             4nm19cs043",prcd[]="PROCEED";
	bg_color();
	glColor3f(.6, 0, 0);
	drawString(23, 80, 0, GLUT_BITMAP_HELVETICA_18, title);
	drawString(20, 75, 0, GLUT_BITMAP_HELVETICA_18, by);
	glColor3f(0,0,0);
...
void stack() {
	...
	glColor3f(0, 0, 0);
	drawString(20, 50, 0, GLUT_BITMAP_HELVETICA_18, stck1);
...
void push(int q) {
	...
	if (stk_top < stk_size - 1) {
		...
		glColor3f(0, 0, 0);
		stk_top += 1;
		drawString(50, stk_push_pos, 0, GLUT_BITMAP_HELVETICA_18, key);
...
void bg_color() {
	glMatrixMode(GL_PROJECTION);
	glColor3f(1, 0.6, 0.8);   //background color
	glBegin(GL_POLYGON);
	glVertex3f(0.0, 0.0, 0.0);
	glColor3f(0.9, 0.56, 0.84);
	//glColor3f(0.137, 0.137, 0.556);
